	...
	Envio(){
   	...
   	virtual void leer(){
      	...
      	if(getTipoEnvio() != 2){
         	...
         	auxCodigo = leeEntero(" > Codigo de cliente[100-999] : ",100,999,"\t* Codigo no valido"); 
   	...
   	void registrarDestino(char *aux,char *aux1,float &ess,float &add){
      	...
      	if(destino->getTipoDestino() == 1){
         	...
         	add = ((DestinoNacional *)destino)->getTarifaAdicional();
      		...
      		cout << endl;
         	ess = leeReal(" > Precio Estandar[60-300] : ",60.0,300.0,"\t* Precio no valido");
         	add = leeReal(" > Precio Adicional[20-100] : ",20.0,100.0,"\t* Precio no valido");
      	}
   	}
